...
- Imagen: `danixu86/project-zomboid-dedicated-server` con `SELF_MANAGED_MODS=1`. Build-arg `STEAMAPPBRANCH` vacío.
...
- `Mods=` = IDs de `mod.info` separados por `;` (load order). `WorkshopItems=` = IDs numéricos separados por `;`. Prefijo `\` por ID: incierto en 42.20.x, probar sin prefijo primero y documentar el resultado en `docs/mods.md`.
...
- Los scripts son bash con `set -euo pipefail` y pasan `shellcheck`.
