-- Reglas de la partida (sandbox). ESTE ARCHIVO ES UN EJEMPLO con los valores vanilla de
-- Project Zomboid Build 42. El que se usa de verdad es config/servertest_SandboxVars.lua, que
-- no se versiona (esta en .gitignore) porque es propio de cada partida. Para personalizarlas:
--
--   cp config/servertest_SandboxVars.example.lua config/servertest_SandboxVars.lua
--
-- Sin ese archivo, scripts/render-config.sh usa este ejemplo tal cual (reglas vanilla).
-- Cada opcion esta explicada en su lugar; varias quedan fijas al crear el mundo (ver README).
-- La encuesta (tools/encuesta) escribe sobre config/servertest_SandboxVars.lua, nunca sobre este.
SandboxVars = {
    VERSION = 6,
